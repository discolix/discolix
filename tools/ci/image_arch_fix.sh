...
echo "Fixing up metadata for arm image: $image"

mkdir -p $work_dir
cd $work_dir
docker save $image -o image.tar
tar -xvf image.tar >/dev/null
rm image.tar
config=$(jq -r '.[0].Config' manifest.json)
sed -i "s/amd64/$arch/g" $config
tar --exclude=image.tar -cvf image.tar . >/dev/null
docker load -i image.tar >/dev/null  # also rehashes image
echo "Reloaded image: $image"
cd ..
rm -rf $work_dir
